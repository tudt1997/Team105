...
from image_augmentation import img_aug
...
class YoloLoss():
    def __call__(self, out, label):
        ...
        for i in range(len(out)):
            ...
            x_hat, y_hat, w_hat, h_hat, p_hat = label[i,0,:,:], label[i,1,:,:], label[i,2,:,:], label[i,3,:,:], label[i,4,:,:]

            # print(x.shape, y.shape, x_hat.shape, y_hat.shape)
            f1 = 5 * torch.sum(p_hat * ((x - x_hat) ** 2 + (y - y_hat) ** 2))
            f2 = 5 * torch.sum(p_hat * ((torch.sqrt(w) - torch.sqrt(w_hat)) ** 2 + (torch.sqrt(h) - torch.sqrt(h_hat)) ** 2))
...
def train():
    ...
    net.load_state_dict(torch.load('./param3/param3_epoch4.txt'))
    print('loaded model')
    net.eval()
    ...
    train_df = pandas.read_csv('./csv/img_label_bbox.csv')

    trainset = SignDataset(df=train_df, img_path='./train_image/', label_path='./saved_label_train/',
                           bbox_path='./label_train/', transform=generic_transform)
    ...
    for epoch in range(5):
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            if i % 10 == 9:
                ...
                e = epoch if (epoch % 2 == 0) else epoch - 1
                torch.save(net.state_dict(), './param4/param4_epoch{}.txt'.format(e))
                print("SAVED!!!")
